...
BASEPATH_SUPPLIER_TEXT_DESCRIPTIONS = os.path.expanduser('~') + '/supplier-data/descriptions/'
list_text_files = os.listdir(BASEPATH_SUPPLIER_TEXT_DESCRIPTIONS)

BASEPATH_SUPPLIER_IMAGES = os.path.expanduser('~') + '/supplier-data/images/'
list_image_files = os.listdir(BASEPATH_SUPPLIER_IMAGES)
list_images = [image_name for image_name in list_image_files if '.jpeg' in image_name]


list = []
for text_file in list_text_files:
	with open(BASEPATH_SUPPLIER_TEXT_DESCRIPTIONS + text_file, 'r') as f:
